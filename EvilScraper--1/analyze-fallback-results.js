  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error(`${colors.red}Failed to parse fixed JSON data:${colors.reset}`, finalError.message);
        return null;
      }
...
if (!fs.existsSync(filePath)) {
  console.error(`${colors.red}Error: File not found at path "${filePath}". Please provide a valid file path.${colors.reset}`);
  process.exit(1);
}
