 // @ts-ignore: Module '@nestjs/common' not found.
...
export class ProviderHealthMonitor {
   ...
   public updateProviderHealth(
       ...
       const totalRequests = this.providerHealth.get(name).recentRequests + 1;
       const totalErrors = this.providerHealth.get(name).recentErrors + (success ? 0 : 1);
       
       this.updateRecentCounts(this.providerHealth.get(name), totalRequests, totalErrors);
       this.updateRates(this.providerHealth.get(name));
       this.updateLatency(this.providerHealth.get(name), success, latency);
       
       if (success) {
           this.providerHealth.get(name).lastUsed = new Date();
       } else {
           this.providerHealth.get(name).lastError = new Date();
           if (errorType) {
               this.logger.warn(`Service provider ${name} encountered an error: ${errorType}`);
           }
       }
       
       this.updateAvailability(this.providerHealth.get(name), name);
   }
   ...
   private updateAvailability(health: ProviderHealth, name: string): void {
    ...
    public resetStats(): void {
        ...
        for (const [name] of this.providerHealth.entries()) {
            this.providerHealth.set(name, {
                name,
                available: true,
                successRate: 1.0,
    ...
    public getProvidersByScore(scoreFunction: (health: ProviderHealth) => number): ProviderHealth[] {
        return Array.from(this.providerHealth.values()).sort((a, b) => scoreFunction(b) - scoreFunction(a));
    }
}
