...
describe('AIGatewayEnhancer - Cost Strategy', () => {
  ...
  let originalProcessWithSmartFallback: any;
  ...
  beforeAll(async () => {
    ...
    mockSelectionStrategy = {
      selectBestProvider: jest.fn().mockImplementation((taskType, strategy) => {
        let selectedProvider = 'openai'; // Default to OpenAI (fastest)
        
        if (strategy === SelectionStrategy.COST_OPTIMIZED) {
          selectedProvider = 'ollama'; // Use Ollama for cost-optimized strategy
        }
    ...
    originalProcessWithSmartFallback = gateway.processWithSmartFallback;
  });
  ...
  beforeEach(() => {
    ...
    gateway.processWithSmartFallback = originalProcessWithSmartFallback;
  });

  it('should prefer cheaper providers in cost strategy', async () => {
    const inputs = ['Prompt A', 'Prompt B'];

    // Mock the processWithSmartFallback method directly
    jest.spyOn(gateway, 'processWithSmartFallback').mockImplementation((taskType, input, options) => {
    ...
    const results = await gateway.processBatchWithSmartFallback(inputs, 'text-generation', 'cost');
    ...
    expect(gateway.processWithSmartFallback).toHaveBeenCalledTimes(2);
    expect(gateway.processWithSmartFallback).toHaveBeenCalledWith(
      'text-generation',
      expect.anything(),
  ...
  it('should fallback to next cheapest provider when cheaper provider fails', async () => {
    ...
    const processWithSmartFallbackSpy = jest.spyOn(gateway, 'processWithSmartFallback');
