...
function parseJsonFromString(input: string): any {
  ...
  try {
    ...
    log(`JSON parsing failed: ${e?.message}, trying regex extraction`);
    const match = clean.match(/\{[\s\S]*\}/);
    if (match) {
      try {
        return JSON.parse(match[0]);
      } catch (e2) {
        log(`JSON object parsing failed: ${e2?.message}`);
      }
    } else {
...
function parseAIResponse(response: string | any, situation: string, options: string[]): Decision {
  ...
  if (typeof response === 'string') {
    jsonResult = parseJsonFromString(response);
    if (!jsonResult) {
      return defaultErrorDecision;
    }
  } else {
  ...
  return {
    ...
    confidence: (() => {
      const conf = parseFloat(jsonResult?.confidence);
      if (isNaN(conf)) {
        return 0.5;
      } else {
  ...
  class AIResponseSimulationServer {
    ...
    private handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
      ...
      req.on('end', () => {
        const { responseData, status } = this.processRequest(req);
        this.sendResponse(res, status, responseData);
      });
    }
    
    private processRequest(req: http.IncomingMessage): { responseData: string, status: number } {
      const url = req.url || '';
      const responseType = url.startsWith('/') ? url.substring(1) : url;
      if (Object.prototype.hasOwnProperty.call(ERROR_RESPONSES, responseType)) {
        return { responseData: ERROR_RESPONSES[responseType], status: 200 };
      } else if (responseType === 'validResponse') {
        const validResponse = JSON.stringify({ action: "option1", reason: "Valid response", confidence: 0.8 });
        return { responseData: validResponse, status: 200 };
      }
      return { responseData: 'Not Found', status: 404 };
    }
    
    private sendResponse(res: http.ServerResponse, status: number, responseData: string): void {
