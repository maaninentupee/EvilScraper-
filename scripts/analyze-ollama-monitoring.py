...
# Memory pressure
high_mem_usage = df[df['Memory Usage %'] > 95]
...
if 'Ollama CPU %' in df.columns and df['Ollama CPU %'].notna().any():
    ollama_cpu = pd.to_numeric(df['Ollama CPU %'], errors='coerce')
...
if 'Ollama CPU %' in correlation_matrix.columns:
    print("\nFactors most correlated with Ollama CPU usage:")
    cpu_corr = correlation_matrix['Ollama CPU %'].sort_values(ascending=False)
...
try:
    ...
    if 'Ollama CPU %' in df.columns and df['Ollama CPU %'].notna().any():
        axs[0].plot(df['Datetime'], pd.to_numeric(df['Ollama CPU %'], errors='coerce'), label='Ollama CPU %')
