...
function updateTestResults(result) {
  ...
  summary.totalTests++;
  summary.successfulTests += result.success ? 1 : 0;
  summary.failedTests += result.success ? 0 : 1;
  summary.fallbacksTriggered += result.fallbackTriggered ? 1 : 0;
  summary.averageResponseTime = (summary.averageResponseTime * (summary.totalTests - 1) + result.responseTime) / summary.totalTests;
  
  const providerStats = testResults.providerStats;
  providerStats[providerKey] = providerStats[providerKey] || { totalCalls: 0, successfulCalls: 0, failedCalls: 0, averageResponseTime: 0 };
  providerStats[providerKey].totalCalls++;
  providerStats[providerKey].successfulCalls += result.success ? 1 : 0;
  providerStats[providerKey].failedCalls += result.success ? 0 : 1;
  providerStats[providerKey].averageResponseTime =
    (providerStats[providerKey].averageResponseTime * (providerStats[providerKey].totalCalls - 1) + result.responseTime) / providerStats[providerKey].totalCalls;
  
  if (!result.success) {
    const errorType = result.errorType || 'unknown';
    testResults.errorTypeStats[errorType] = testResults.errorTypeStats[errorType] || { count: 0, fallbacksTriggered: 0 };
    testResults.errorTypeStats[errorType].count++;
    testResults.errorTypeStats[errorType].fallbacksTriggered += result.fallbackTriggered ? 1 : 0;
  }
