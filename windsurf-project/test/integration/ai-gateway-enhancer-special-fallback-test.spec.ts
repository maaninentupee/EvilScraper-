...
describe('AIGatewayEnhancer Special Fallback Integration Test', () => {
  ...
  it('should skip unavailable provider and try the next one', async () => {
    ...
    // First call to processAIRequest fails with provider_unavailable error
    const processAISpy = jest.spyOn(aiGateway, 'processAIRequest')
      .mockImplementationOnce(async () => {
        throw new Error('Provider unavailable');
      });
    
    // Mock the process method to throw an error for the first provider
    const processSpy = jest.spyOn(aiGatewayEnhancer, 'process')
      .mockImplementationOnce(async () => {
        throw new Error('Provider unavailable');
      })
      // Then succeed with the second provider
      .mockImplementationOnce(async () => ({
        success: true,
